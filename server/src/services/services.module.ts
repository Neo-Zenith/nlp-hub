...
export class ServiceModule {
    configure(consumer: MiddlewareConsumer) {
        consumer.apply(CreateServiceMiddleware).forRoutes({
            path: '/services',
            method: RequestMethod.POST,
        })

        consumer.apply(UpdateServiceMiddleware).forRoutes({
            path: '/services',
            method: RequestMethod.PUT,
        })

        consumer.apply(CreateEndpointMiddleare).forRoutes({
            path: '/services/:type/:version/endpoints',
            method: RequestMethod.POST,
        })

        consumer.apply(UpdateEndpointMiddleware).forRoutes({
