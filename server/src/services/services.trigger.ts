import { ServiceEndpointModel, ServiceModel, ServiceSchema } from './services.model'

// Triggers for NlpSchema
export function ServiceTrigger() {
    // Pre-save trigger
    ServiceSchema.pre('save', async function (next) {
        const highestVersion = await ServiceModel.findOne({ type: this.type }).sort({ version: -1 })
